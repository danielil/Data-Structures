 ...
 */
nodeData::nodeData(const string& token)
    : tokenFrequency(0)
{
    setToken(token);
}
 ...
 */
void nodeData::setToken(const string& token)
{
    if (!token.empty())
    {
        tokenData = token;
 ...
 *      > 0 : local token < target data
 */
int nodeData::compare(const string& token) const
{
 ...
 */
bool nodeData::operator==(const string& token) const
{
 ...
 */
void nodeData::show(ostream& output, bool details) const
{
    if (details)
    {
