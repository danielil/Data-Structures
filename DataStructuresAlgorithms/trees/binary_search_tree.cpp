...
namespace dsa
{
    ...
    BinTree::BinTree(const BinTree &source) : root(nullptr)
    {
        *this = source;
    }
    ...
    {
        return findNode(root, targetData, nullptr);
    }
    ...
    bool BinTree::insertHelper(Node* current, NodeData* data)
    {
        ...
        if ((current != nullptr) && (current->data != nullptr) && (*(current->data) != *data))
        {
    ...
    void BinTree::sideways(Node* current, int level) const
    {
        if (current != nullptr)
        {
            ...
            std::cout << *current->data << std::endl; // display information of object
