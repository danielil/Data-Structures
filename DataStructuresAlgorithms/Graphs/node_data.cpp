...
namespace dsa
{
    ...
    NodeData::NodeData(const std::string& s) { data = s; }    // cast string to NodeData
    ...
    bool NodeData::setData( std::istream& infile) {
        getline(infile, data);
        return !infile.eof();       // eof function is true when eof char is read
    }

    //-------------------------- operator<< --------------------------------------
    std::ostream& operator<<( std::ostream& output, const NodeData& nd) {
        output << nd.data;
        return output;
    }
}
